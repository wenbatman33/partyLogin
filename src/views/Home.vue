    ...
    .container-fluid(v-if='setting.page !=="0" && admin.logined')
      ...
      .reportTotalData.p-2(v-if='setting.page=="3"')
        .w-100.p-2.text-center
          ...
          button.m-1(:class='[selectDep === "ICS" ? "wButton" : "gButton"]', @click='selectDep="ICS"')
            span ICS:{{icsAllMembaer}} 
          button.m-1(:class='[selectDep === "HG" ? "wButton" : "gButton"]', @click='selectDep="HG"')
            span HG:{{hgAllMembaer}}  
          button.m-1(:class='[selectDep === "HS" ? "wButton" : "gButton"]', @click='selectDep="HS"')
            span HS:{{lsAllMembaer}} 
          button.m-1(:class='[selectDep === "HB" ? "wButton" : "gButton"]', @click='selectDep="HB"')
            ...
            .row.no-gutters
              ...
              .col-11
                ...
                p
                  span {{user.member.ex}} 
                  span.grayText {{user.datetime}}
                  span.float-right(@click='deleteMember(user.id)', v-if='admin.name==authority') 刪除
</template>
<script>
import { db } from '../main';
...
export default {
  ...
  computed: {
    ...
    kiAllMembaer(){
      let members = 0
      let filterDepartment = this.firestoreMember.filter(function(item, index, array){
        if (item.member.gid.indexOf('KI')>=0){
          members +=1;
        }
      });
      return members;
